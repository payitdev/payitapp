...
describe('Particle Auth User Info Normalizer Unit Tests', () => {
  it('1. SHOULD normalize standard Particle useConnect response with wallets array', () => {
    const rawInfo = {
      wallets: [{ uuid: 'w-1', chain_name: 'evm_chain' }],
    ...
    const normalized = formatParticleUserInfo(rawInfo, 'google', '0x1234567890abcdef1234567890abcdef12345678');
  ...
  it('2. SHOULD unwrap nested rawUserInfo.userInfo payload structures', () => {
    const nestedRawInfo = {
      userInfo: {
        wallets: [{ uuid: 'w-2', chain_name: 'evm_chain' }],
    ...
    const normalized = formatParticleUserInfo(nestedRawInfo, 'google', '0x9999888877776666555544443333222211110000');
  ...
  it('3. SHOULD prioritize default user email input if provided for email auth', () => {
    const rawInfo = {
      uuid: 'session_token_999',
    };

    const normalized = formatParticleUserInfo(rawInfo, 'email', '0x7777888899990000111122223333444455556666', undefined, 'finance@company.com');
