...
export default function App() {
  ...
  const [authMethod, setAuthMethod] = useState<'google' | 'apple' | 'email'>('google');
  ...
  }, [accountType, currentUser, activeEntity?.id]);
  ...
  const handleParticleSocialSignIn = async (provider: 'google' | 'apple' | 'email') => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Particle Auth sign-in failed');
      ...
      const userObj = data.user || data.session;
      if (!userObj) throw new Error('Invalid user session returned from Particle Auth server');
  ...
  const handleSubmitKyc = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(data.message || 'Identity details submitted to Nuvion successfully!');
  ...
  const handleSendSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // C3 Remediation: For crypto sends, broadcast on-chain via Particle SDK before calling backend
      if (sendModeTab === 'crypto') {
        setSendStatusMsg({ type: 'warning', text: 'Confirming transaction on-chain via Particle Universal Account...' });
  ...
  const handleFreezeVirtualCard = async (cardId: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/cards/freeze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session: { userId, activeEntityId: activeEntity.id, userEntityIds: [activeEntity.id] },
  ...
  const handleCheckUsername = async (val: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/users/check-username?username=${encodeURIComponent(val)}`);
      const data = await res.json();
  ...
  const handleUpdateUsername = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/users/update-username`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session: { userId: currentUser.userId, activeEntityId: activeEntity.id, userEntityIds: [activeEntity.id] },
  ...
  // UNAUTHENTICATED PARTICLE NETWORK AUTHENTICATION VIEW
  if (!currentUser) {
    return (
      <div style={{ background: '#0F172A', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        <div className="phone" style={{ height: 'auto', maxHeight: 'none', padding: 28, textAlign: 'center' }}>
          <div className="logo" style={{ fontSize: 26, justifyContent: 'center', marginBottom: 8 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 24 }}>
            Modern multi-currency payments for personal spending &amp; business treasury
          </div>

          {authError && (
            <div style={{ background: '#FEF2F2', border: '1px solid var(--danger)', color: 'var(--danger)', padding: 12, borderRadius: 12, fontSize: 12, marginBottom: 16 }}>
              {authError}
            </div>
          )}

          {/* Particle Network Social Login Options */}
          ...
          <div style={{ position: 'relative', margin: '20px 0', fontSize: 12, color: 'var(--muted)' }}>
            <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: 1, background: 'var(--border)', zIndex: 0 }} />
            <span style={{ position: 'relative', background: '#fff', padding: '0 10px', zIndex: 1 }}>OR</span>
          </div>
          ...
          <form onSubmit={e => { e.preventDefault(); handleParticleSocialSignIn('email'); }} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button type="submit" disabled={isLoggingIn} className="cta">
              {isLoggingIn ? 'Verifying with Particle Auth...' : 'Sign in with Email'}
            </button>
          </form>

          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 24, lineHeight: 1.5 }}>
            Particle Network derives non-custodial salt-isolated Universal Accounts with zero exposure of seed phrases or gas fees.
          </div>
        </div>
      </div>
    );
            ...
            {(!activeEntity?.nuvionStatus || activeEntity.nuvionStatus === 'incomplete') && (
              <div style={{ background: '#FFF7ED', border: '1px solid #FDBA74', borderRadius: 16, padding: 14, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#9A3412' }}>Verify Identity (KYC/KYB)</div>
                    <div style={{ fontSize: 11, color: '#C2410C' }}>Unlock multi-currency accounts &amp; transfers</div>
                  </div>
                </div>
                <button onClick={() => setShowKycModal(true)} className="chip" style={{ cursor: 'pointer', background: '#EA580C', color: '#fff' }}>
                  Verify
                </button>
              </div>
            )}
                  ...
                  <div style={{ background: 'var(--surface-alt)', border: '1px solid var(--border)', borderRadius: 14, padding: 14 }}>
                    ...
                    <div className="key-address" style={{ background: '#fff', padding: 10, borderRadius: 10, border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <button
                        ...
                        style={{ marginLeft: 8 }}
                        onClick={() => {
                          navigator.clipboard.writeText((activeEntity as any)?.solanaAddress || '');
