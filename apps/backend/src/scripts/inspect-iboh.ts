...
async function inspectIboh() {
  console.log('--- INSPECTING USER: Iboh Igboze (payitdev@gmail.com) ---');
  
  const userRows = await db.select().from(users).where(eq(users.email, 'payitdev@gmail.com'));
  if (userRows.length === 0) {
    console.log('User payitdev@gmail.com NOT found in users table!');
    process.exit(0);
  }
  ...
  for (const ent of entityRows) {
    console.log(`\n- Entity ID: ${ent.id} | Kind: ${ent.kind} | LegalName: "${ent.legalName}"`);
...
inspectIboh().catch(console.error).finally(() => process.exit(0));
